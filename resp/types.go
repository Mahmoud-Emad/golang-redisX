...
func NewBulkString(str string) (BulkString, error) {
	...
	if strLen > MaxBulkSizeLength {
		return BulkString{}, errors.New("Cannot allocate a string of length " + fmt.Sprint(strLen) + " because it exceeds max allowed size of " + MaxBulkSizeAsHumanReadableValue)
	}
	return BulkString{
		value:       str,
		isNullValue: false,
